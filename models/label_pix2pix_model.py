...
class LabelPix2PixModel(torch.nn.Module):
    ...
    def forward(self, data, mode):
        input_semantics, real_image, input1_semantics, real1_image = self.preprocess_input(data)
        ...
        if mode == 'generator':
            ...
            with torch.no_grad():
                fake_image, _ = self.generate_fake(input_semantics, real_image, input1_semantics, real1_image)
            return fake_image
        else:
    ...
    def preprocess_input(self, data):
        ...
        return input_semantics, data['image'], input1_semantics, data['image1']
    ...
    def generate_fake(self, input_semantics, real_image, input1_semantics, real1_image, compute_kld_loss=False):
        z = None
        KLD_loss = None
        if self.opt.use_vae:
            z, mu, logvar = self.encode_z(real_image)
        ...
        fake_image = self.netG((input_semantics+input1_semantics)/2, z=z)
